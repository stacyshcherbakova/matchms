...
def test_default_score_dtype_matches_other_cosine_scores():
    ...
    bc = BlinkCosine(tolerance=5.0, bin_width=1.0, prefilter=False)
    cg = CosineGreedy(tolerance=5.0)

    assert np.dtype(bc.score_datatype) == np.dtype(cg.score_datatype)
    assert bc.pair(s1, s2).dtype == cg.pair(s1, s2).dtype
    assert bc.matrix([s1], [s2], array_type="numpy").dtype == np.float64
    assert bc.matrix([s1], [s2], array_type="sparse").dtype == np.float64
...
def test_dtype_parameter_applies_to_pair_and_matrix(dtype, use_numba):
    ...
    sim = BlinkCosine(tolerance=5.0, bin_width=1.0, prefilter=False, intensity_power=0.5,
                      use_numba=use_numba, dtype=dtype)

    out = sim.pair(refs[0], qrys[0])
    assert out["score"].dtype == dtype
    assert sim.matrix(refs, qrys, array_type="numpy").dtype == dtype
    assert sim.matrix(refs, qrys, array_type="sparse").dtype == dtype
...
def test_float32_and_float64_scores_agree():
    ...
    f32 = BlinkCosine(tolerance=5.0, bin_width=1.0, prefilter=False, dtype=np.float32)
    f64 = BlinkCosine(tolerance=5.0, bin_width=1.0, prefilter=False, dtype=np.float64)

    assert float(f32.pair(s1, s2)["score"]) == pytest.approx(float(f64.pair(s1, s2)["score"]), 1e-6)
    assert np.allclose(f32.matrix([s1, s2], [s1, s2]), f64.matrix([s1, s2], [s1, s2]), atol=1e-6)
